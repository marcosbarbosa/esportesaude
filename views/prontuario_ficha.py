...
def _dialog_arquivar_aluno(aluno):
    ...
    nome_safe = _html.escape(aluno.get("nome", ""))
    st.markdown(
        f"<p style='margin-bottom:6px;'>Você está prestes a arquivar <strong>{nome_safe}</strong>."
...
def excluir_atestado_temporario_local(atestado_id):
    try:
        ...
        # Reflete a remoção imediatamente no prontuário, chamada e tela inicial.
        from database import _inv_alunos
        _inv_alunos()
...
def render_cabecalho_aluno(aluno):
    ...
    # 📅 Banner de validade — fonte única: status_atestado()
    _status_ate_ficha = status_atestado(
        get_atestados_temporarios(aluno["id"]),
        bloqueado_manual=_bloqueado_atestado,
    )
    _vb_col, _vb_btn = st.columns([5, 1], vertical_alignment="center")
    with _vb_col:
        _f_bg = _status_ate_ficha["atestado_fundo"]
        _f_cor = _status_ate_ficha["atestado_cor"]
        _f_ico = _status_ate_ficha["atestado_icone"]
        _f_data = _status_ate_ficha["data_vencimento_formatada"]
        _f_txt = _status_ate_ficha["rotulo_atestado"]
        if _status_ate_ficha["atestado_bloqueado_manual"]:
            _f_txt += " — participação bloqueada manualmente"
        st.markdown(
            f"<div style='background:{_f_bg};border-left:5px solid {_f_cor};"
            f"padding:10px 16px;border-radius:6px;display:flex;align-items:center;gap:10px;'>"
            f"<span style='font-size:18px;'>{_f_ico}</span>"
            f"<span style='color:{_f_cor};font-weight:700;font-size:13px;'>"
            f"Atestado de Aptidão — validade: <u>{_f_data}</u> — {_f_txt}</span></div>",
            unsafe_allow_html=True,
        )
    with _vb_btn:
    ...
    with cp_word:
        ...
        if aluno.get("status", "Ativo") != "Inativo":
            ...
            if st.button("♻️ Reativar Aluno", type="primary", use_container_width=True):
                ...
                ok, _ = alterar_status_aluno_local(aluno["id"], "Ativo", operador=_operador_reat)
                if ok:
                    aluno["status"] = "Ativo"
                    st.rerun()
    ...
    if aluno.get("status", "Ativo") == "Inativo":
        ...
        _icone_motivo = {
            "Óbito": "⚰️", "Desistência": "🚪", "Transferência": "🔄",
            "Conclusão": "🎓", "Outro": "📋",
        }.get(_motivo, "📋")
        ...
        if _hist:
            ...
            with st.expander(f"🕐 Histórico de Status ({len(_hist)} registro{'s' if len(_hist) != 1 else ''})", expanded=False):
                ...
                for _ev in _hist:
                    ...
                    # Detalhes extras (motivo, data de saída, obs, operador)
                    _extras = []
        ...
        if btn_doc_top or btn_doc_bot:
            if novo_rg or novo_rec:
                with st.spinner("A guardar na nuvem..."):
                    final_rg, final_rec = url_rg, url_rec
                    upload_docs_ok = True
    ...
    with st.expander("📋 Histórico de Atestados", expanded=True):
        with st.form("form_novo_atestado_temp", clear_on_submit=True):
            ...
            data_atestado = c_dt.date_input(
                "Data do Atestado:", datetime.date.today(), format="DD/MM/YYYY"
            )
        ...
        if atestados_hist is not None and not atestados_hist.empty:
            for _, at_temp in atestados_hist.iterrows():
                with st.container(border=True):
                    ...
                    motivo_d = str(at_temp.get("motivo") or "")
                    c_ht1.markdown(
            ...
            st.caption("Nenhum atestado arquivado.")
